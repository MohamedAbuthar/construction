...
// EVM Chart Component - REMOVED the legend from here
function EVMChart() {
        ...
        if (ctx) {
          ...
          chartInstance.current = new Chart(ctx, {
            ...
            options: {
              ...
              plugins: {
                legend: {
                  display: false, // Keep this false since we're using custom legend
                },
                tooltip: {
                  mode: "index",
                  intersect: false,
  ...
  return (
    <div className="w-full h-full">
      <canvas ref={chartRef} />
      {/* REMOVED the duplicate legend from here */}
    </div>
  )
}
  ...
  return (
    <div className="space-y-6 lg:space-y-8 mx-2 sm:mx-3 md:mx-4 lg:mx-6 xl:mx-8 my-4 sm:my-6 lg:my-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 min-h-32">
          <h3 className="text-gray-600 text-sm sm:text-base">Milestones</h3>
          <p className="text-2xl sm:text-3xl font-bold mt-2">{project.milestones}</p>
          <p className="text-sm sm:text-base text-gray-500 mt-1">{project.completedMilestones} completed</p>
        </div>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 min-h-32">
          <h3 className="text-gray-600 text-sm sm:text-base">Activities</h3>
          <p className="text-2xl sm:text-3xl font-bold mt-2">{project.activities}</p>
          <p className="text-sm sm:text-base text-gray-500 mt-1">1 in progress</p>
        </div>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 min-w-0 min-h-32">
          <h3 className="text-gray-600 text-sm sm:text-base">Budget</h3>
          <p className="text-2xl sm:text-3xl font-bold mt-2 break-words">{project.budget}</p>
          <p className="text-sm sm:text-base text-gray-500 mt-1">Total allocated</p>
        </div>
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6 min-h-32">
          <h3 className="text-gray-600 text-sm sm:text-base">Purchase Orders</h3>
          <p className="text-2xl sm:text-3xl font-bold mt-2">{project.purchaseOrders}</p>
          <p className="text-sm sm:text-base text-gray-500 mt-1">1 received</p>
        </div>
      </div>
                  ...
                  <table className="w-full min-w-[600px]">
                    <thead>
                      <tr className="border-b border-gray-200 bg-gray-50">
                        <th className="text-left py-3 px-3 sm:px-6 font-semibold text-gray-700 text-base sm:text-lg">
                          Name
                        </th>
                        <th className="text-left py-3 px-3 sm:px-6 font-semibold text-gray-700 text-base sm:text-lg">
                          Budget
                    ...
                    <tbody>
                      {project.activitiesData.map((activity, index) => (
                        <tr key={index} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-3 px-3 sm:px-6">
                            <span
                              className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-sm font-medium bg-white border border-gray-300 text-gray-800 ${getStatusColor(activity.status)}`}
                            >
                              {activity.status}
                            </span>
                          </td>
          ...
          {activeTab === "evm" && (
            <div className="space-y-4 sm:space-y-6 lg:space-y-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
                <div className="bg-red-100 rounded-lg shadow-sm border border-red-200 p-4 sm:p-5 lg:p-6 min-w-0 min-h-40">
                  <div className="flex flex-col h-full justify-between">
                    <div className="min-w-0">
                      <h3 className="text-gray-600 font-medium text-xs sm:text-sm lg:text-base leading-tight">
                        Cost variance (CV)
                      </h3>
                      <p className="text-red-500 text-lg sm:text-xl lg:text-2xl font-bold mt-2 sm:mt-3 break-words">
                        ₹30,00,000
                      </p>
                    </div>
                    <TrendingDown className="w-5 h-5 sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-red-500 flex-shrink-0 mt-3" />
                  </div>
                </div>
                <div className="bg-red-100 rounded-lg shadow-sm border border-red-200 p-4 sm:p-5 lg:p-6 min-w-0 min-h-40">
                  <div className="flex flex-col h-full justify-between">
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 lg:p-6 relative">
                ...
                <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3 sm:mb-4 lg:mb-6 gap-3 sm:gap-4">
                  ...
                  <div className="w-full sm:w-auto">
                    <button
                      ...
                      className="flex items-center justify-center sm:justify-start space-x-1 sm:space-x-2 bg-blue-600 text-white px-2 sm:px-3 lg:px-4 py-1.5 sm:py-2 rounded-md sm:rounded-lg hover:bg-blue-700 transition-colors shadow-sm sm:shadow-md text-xs sm:text-sm lg:text-base whitespace-nowrap w-full sm:w-auto"
                    >
                ...
                {/* EVM Chart Legend */}
                <div className="flex flex-col items-center gap-2 sm:gap-3 lg:gap-4 mt-3 sm:mt-4 lg:mt-6">
                  <div className="flex flex-wrap justify-center items-center gap-3 sm:gap-4 lg:gap-6">
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 lg:w-3 lg:h-3 rounded-full bg-blue-500 flex-shrink-0"></div>
                      <span className="text-xs sm:text-sm lg:text-base text-gray-600">Planned Value</span>
                    </div>
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 lg:w-3 lg:h-3 rounded-full bg-green-500 flex-shrink-0"></div>
                      <span className="text-xs sm:text-sm lg:text-base text-gray-600">Earned Value</span>
                    </div>
                    <div className="flex items-center space-x-1 sm:space-x-2">
                      <div className="w-2 h-2 sm:w-2.5 sm:h-2.5 lg:w-3 lg:h-3 rounded-full bg-red-500 flex-shrink-0"></div>
                      <span className="text-xs sm:text-sm lg:text-base text-gray-600">Actual Cost</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 p-3 sm:p-4 lg:p-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
                  <div>
                    <h4 className="font-semibold text-gray-700 mb-2 sm:mb-3 text-xs sm:text-sm lg:text-base">
                      Variance Formulas
                    </h4>
                    <div className="space-y-2 sm:space-y-3">
                      <div>
                        <p className="text-gray-900 font-medium text-xs sm:text-sm lg:text-base">Cost Variance (CV)</p>
      ...
      <AddMilestoneDialog isOpen={isMilestoneDialogOpen} onClose={() => setIsMilestoneDialogOpen(false)} />

      {/* Add Activity Dialog */}
      <AddActivityDialog isOpen={isActivityDialogOpen} onClose={() => setIsActivityDialogOpen(false)} />

      {/* Add Snapshot Dialog */}
      <AddSnapshotDialog isOpen={isSnapshotDialogOpen} onClose={() => setIsSnapshotDialogOpen(false)} />
    </div>
  )
}
